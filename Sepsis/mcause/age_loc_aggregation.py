...
NUM_DRAWS = CONF.get_id("draw_num")
...
COD_AGES = get_cod_ages()["age_group_id"].unique().tolist()


def get_lvl2_lvl0_aggregates(
    df, logger, pop_df=None, draw_cols=EST_COLS, id_cols=DEM_COLS, num_draws=NUM_DRAWS
):
    df = add_population(df, pop_df=pop_df)
    report_if_merge_fail(df, "population", ["age_group_id", "sex_id", "location_id", "year_id"])

    logger.debug("Creating global aggregates")
    global_df = df.copy()
    global_df["location_id"] = 1
    global_df = scale_location_aggregates(
        global_df, pop_df=pop_df, draw_cols=draw_cols, id_cols=id_cols, num_draws=num_draws
    )

    logger.debug("Creating region aggregates")
    region_df = df.copy()
    region_df["location_id"] = region_df["region_id"]
    region_df = scale_location_aggregates(
        region_df, pop_df=pop_df, draw_cols=draw_cols, id_cols=id_cols, num_draws=num_draws
    )

    logger.debug("Creating super region aggregates")
    ...
    scaled_locs_df = pd.concat(
        [global_df, region_df, super_region_df], ignore_index=True, sort=True
    )

    return scaled_locs_df


def scale_location_aggregates(
    df, pop_df=None, draw_cols=EST_COLS, id_cols=DEM_COLS, num_draws=NUM_DRAWS
):
    df = df.copy()
    ...
    for col in EST_COLS:
        df["rate_" + col] = df[col] / df["population"]
    ...
    for col in EST_COLS:
        df[col] = df["rate_" + col] * df["population"]

    RATE_COLS = [x for x in df.columns if "rate_" in x]
    df = df.drop(RATE_COLS + ["population"], axis=1)
    return df


def get_country_aggregate(df, lhh, draw_cols=EST_COLS, id_cols=DEM_COLS):
...
def aggregate_locs(df, lhh, logger, draw_cols=EST_COLS, id_cols=DEM_COLS, num_draws=NUM_DRAWS):
    df = df.copy()
    assert len(draw_cols) - 1 == num_draws

    logger.debug("Reading in population")
    ...
    if len(set(data_ages) - set(COD_AGES)) > 0:
        logger.debug(f"Aggregating population age groups to match: {data_ages} ids")
    ...
    lvl2_lvl0_df = get_lvl2_lvl0_aggregates(
        df, logger, pop_df=pop_df, draw_cols=draw_cols, id_cols=id_cols, num_draws=num_draws
    )
...
def create_age_standardized_rates(
    df, draw_cols, dem_cols, location_set=CONF.get_id("location_set")
):
    ...
    age_weight_dict = (
        age_weight_df.drop_duplicates(["age_group_id", "age_group_weight_value"])
    ...
    if len(null_pop_df) > 0:
        null_pop_df = add_population(
            null_pop_df, pop_run_id=pop_run_id, location_set_id=location_set, **cache_kwargs
        )
...
def aggregate_ages(df):
    age_ids = [22, 1, 158, 23, 159, 24, 25, 26, 21, 28, 157, 42, 162]
...
def aggregate_sexes(df):
    ...
    all_sex_df = df.groupby(group_cols, as_index=False)[EST_COLS].sum()
    all_sex_df["sex_id"] = 3
    return all_sex_df


def main_age_loc(int_cause, description, end_product, cause_id, year_id, logger=None):
    if logger == None:
        logger = setup_logger(f"age_loc_aggregation_{int_cause}_{year_id}_{cause_id}")
    ...
    if end_product in ["mortality", "incidence", "attributable_burden"]:
        logger.debug("Creating age aggregates")
        agg_age_df = aggregate_ages(df)
        asr_df = create_age_standardized_rates(df, EST_COLS, DEM_COLS)
...
if __name__ == "__main__":
    ...
    main_age_loc(int_cause, description, end_product, cause_id, year_id)
