...
from cod_prep.utils.cod_db_tools import add_tbl_metadata, get_function_results
...
def get_datasets(
    ...
    if is_active is not None:
        assert isinstance(is_active, bool), "Pass either True or False to is_active"
    ...
    proj_datasets = get_mcause_project_datasets(**cache_options).query(
        f"project_id == {project_id}"
    )
    datasets = datasets.merge(proj_datasets, how="left", on=["nid", "extract_type_id"])
    datasets["is_active"] = datasets["is_active"].fillna(0)

    add_cols = ["source", "data_type_id", "code_system_id", "parent_nid"]
    datasets = add_nid_metadata(datasets, add_cols, **cache_options)
    for need_col in ["source", "data_type_id", "code_system_id"]:
    ...
    if len(datasets) == 0:
        raise AssertionError(
            "Given dataset filters produced no " "datasets: \n{}".format(dataset_filters)
        )
...
def get_mcause_project_datasets(**cache_kwargs):
    nid_query = """SELECT project_id, nid, extract_type_id, is_active
                    FROM cod.mcause_project_datasets"""

    function = ezfuncs.query
    args = [nid_query]
    kwargs = {"conn_def": CONF.get_database_setup("db")}
    cache_name = "mcause_project_datasets"
...
def add_nid_metadata(
    df, add_cols, merge_col=["nid", "extract_type_id"], nid_meta_df=None, **cache_kwargs
):
    if nid_meta_df is not None:
        tbl_or_function = nid_meta_df
    else:
        tbl_or_function = get_nid_metadata
    return add_tbl_metadata(tbl_or_function, df, add_cols, merge_col, **cache_kwargs)


def get_value_from_nid(
    nid,
    extract_type_id,
    value,
    force_rerun=False,
