...
from cod_prep.utils import get_function_results, report_duplicates, report_if_merge_fail

CONF = Configurator("standard")


def _get_sepsis_cause_hierarchy(int_cause, cause_set_version_id, **cache_kwargs):
    ...
    nested_name_dict = (
        df[["Nested code ", "acause"]]
        .dropna()
        .drop_duplicates()
        .set_index("Nested code ")["acause"]
        .to_dict()
    )
    ...
    chh = get_current_cause_hierarchy(cause_set_version_id=cause_set_version_id, **cache_kwargs)
    chh = chh.loc[
        ((chh["yld_only"] != 1) & (chh["parent_id"] != 294) & (chh["is_estimate"] == 1))
        | ((chh["yld_only"] != 1) & (chh["parent_id"] != 294) & (chh["cause_id"].isin([360, 462])))
    ]
    ...
    chh.loc[chh["cause_id"].isin([1048, 1049]), "level_1"] = 1000
    chh.loc[chh["cause_id"].isin(list(range(1033, 1040))), "level_1"] = 1100
    chh.loc[chh["cause_id"].isin([1040, 1041, 1042]), "level_1"] = 1800
    chh.loc[chh["cause_id"].isin([1024, 1025]), "level_1"] = 2000
    chh.loc[chh["cause_id"].isin(list(range(1050, 1054))), "level_1"] = 1300
    chh.loc[chh["cause_id"].isin([1032]), "level_1"] = 2300
    chh.loc[chh["cause_id"].isin(list(range(1043, 1048))), "level_1"] = 2500
    ...
    chh.loc[chh["cause_id"] == 624, "level_1"] = 2500
    chh.loc[chh["cause_id"] == 360, "level_1"] = 1200
    chh.loc[chh["cause_id"] == 462, "level_1"] = 1600
...
def _get_int_cause_hierarchy(int_cause, cause_set_version_id, **cache_kwargs):
    chh = get_current_cause_hierarchy(cause_set_version_id=cause_set_version_id, **cache_kwargs)
    chh["level_2"] = chh["cause_id"]
    chh.loc[chh["secret_cause"] == 1, "level_2"] = chh["parent_id"]
...
def get_int_cause_hierarchy(
    int_cause, cause_set_id=4, gbd_round_id=None, cause_set_version_id=None, **cache_kwargs
):
    if not gbd_round_id:
...
def get_most_detailed_inj_causes(
    int_cause, cause_set_version_id=None, cause_set_id=3, gbd_round_id=None, **cache_kwargs
):
    ...
    inj_causes = list(cause_df.loc[cause_df["acause"].str.contains("inj")].cause_id.unique())
...
def agg_secret_causes(df, **cache_kwargs):
    ...
    chh = get_current_cause_hierarchy(cause_set_id=4, gbd_round_id=None, **cache_kwargs)
    sch = chh.loc[(chh["acause"].str.contains("inj")) & (chh["secret_cause"] == 1)]
    ...
    df.loc[(df["secret_cause"] == 1) & (df["cause_id"] != 743), "cause_id"] = df["parent_id"]
    df.loc[(df["secret_cause"] == 1) & (df["cause_id"] != 743), "acause"] = df["acause_parent"]
    df.drop(["secret_cause", "parent_id", "acause_parent"], axis=1, inplace=True)
...
def get_injuries_cause_ids(int_cause, cache_kwargs):
    ...
    d = get_most_detailed_inj_causes(
        int_cause,
...
def combine_cause_sets(end_product, **cache_kwargs):
    ...
    chh = get_current_cause_hierarchy(
        ...
        **cache_kwargs,
    ).query(f"{subset_string}")[["level", "cause_id", "parent_id", "most_detailed"]]
    levels = list(chh.level.unique())
    chh_rep = get_current_cause_hierarchy(
        cause_set_version_id=CONF.get_id("reporting_cause_set_version"),
        cause_set_id=CONF.get_id("reporting_cause_set"),
        **cache_kwargs,
    )
    reporting_aggs = list(
        chh_rep.loc[chh_rep["acause"].str.contains("reporting"), "cause_id"].unique()
    )
    reporting_agg_df = get_current_cause_hierarchy(
        cause_set_version_id=CONF.get_id("reporting_cause_agg_set_version"),
        cause_set_id=CONF.get_id("reporting_cause_agg_set"),
        **cache_kwargs,
    ...
    reporting_agg_df = reporting_agg_df.append(
        {"cause_id": 1008, "parent_id": 1022, "most_detailed": 0}, ignore_index=True
    )
    reporting_agg_df = reporting_agg_df.merge(chh[["cause_id", "level"]], how="left", on="cause_id")
    assert (reporting_agg_df.groupby("parent_id")["level"].nunique() == 1).all()
    reporting_agg_df = reporting_agg_df.append(
        reporting_agg_df[["parent_id", "level"]]
        .drop_duplicates()
...
def get_level_parent_child_cause_dict(end_product, **cache_kwargs):
    ...
    for level in levels:
        ...
        for cause_id in level_df.cause_id.unique():
            children = list(
                chh.query(f"parent_id == {cause_id} & cause_id != {cause_id}").cause_id.unique()
            )
...
def get_child_causes(parent_cause_id, end_product, **cache_kwargs):
    ...
    child_cause_list = list(
        cause_meta_df.query(f"parent_id == {parent_cause_id}")["cause_id"].unique()
    )
...
def get_infsyn_hierarchy(infsyn_set_version="v1"):
    if infsyn_set_version == "v1":
        return pd.read_csv("FILEPATH")
    elif infsyn_set_version == "gram2":
        return pd.read_csv("FILEPATH")
    else:
        return pd.read_csv("FILEPATH")


def get_child_to_available_parent_syndrome(available_parents, infsyn=None, infsyn_set_version="v1"):
    if infsyn is None:
        infsyn = get_infsyn_hierarchy(infsyn_set_version=infsyn_set_version)
...
def get_all_related_syndromes(syndrome, infsyn=None, infsyn_set_version="v1", level=None):
    if infsyn is None:
        infsyn = get_infsyn_hierarchy(infsyn_set_version=infsyn_set_version)
    ars = (
        infsyn.loc[
    ...
    if level is not None:
        lars = (
            infsyn.loc[
                infsyn["infectious_syndrome"].isin(ars) & (infsyn["level"] <= level),
                "infectious_syndrome",
            ]
            .unique()
            .tolist()
...
def prep_child_to_available_parent_name_map(
    cause_meta_df=None, available_cause_names=None, as_dict=False, expect_all=True
):

    if cause_meta_df is None:
        cause_meta_df = pd.read_csv("FILEPATH", encoding="latin-1")

    if available_cause_names is None:
        available_cause_names = cause_meta_df.acause.unique().tolist()
        available_cause_names += ["care"]

    cause_meta_df["cause_name"] = cause_meta_df["acause"]
    cause_meta_df = cause_meta_df.set_index("acause")

    cause_tree = cause_meta_df.path_to_top_parent.str.split(";").apply(pd.Series, 1)
    cause_tree = cause_tree.stack().reset_index()
    cause_tree = cause_tree.rename(columns={"level_1": "par_level", 0: "parent_cause_name"})
    cause_tree["parent_cause_name"] = cause_tree["parent_cause_name"].astype(str)

    cause_availability = {c: 1 for c in available_cause_names}
    cause_tree["is_available"] = cause_tree["parent_cause_name"].map(cause_availability).fillna(0)

    available_cause_map = cause_tree.query("is_available == 1")
    available_cause_map.rename(columns={"acause": "cause_name"}, inplace=True)
    available_cause_map["max_level_available"] = available_cause_map.groupby("cause_name")[
        "par_level"
    ].transform(max)
    available_cause_map = available_cause_map.query("par_level == max_level_available")
    available_cause_map = available_cause_map[["cause_name", "parent_cause_name"]]

    available_cause_map.drop_duplicates(subset=["cause_name"], inplace=True)

    assert not available_cause_map[["cause_name"]].duplicated().any()
    missing = set(cause_meta_df.index) - set(available_cause_map["cause_name"])
    if len(missing) > 0 and expect_all:
        raise AssertionError(
            "Was not able to find parent in given available cause "
            "names list for these cause names: \n{}".format(missing)
        )

    if as_dict:
        available_cause_map = available_cause_map.set_index("cause_name")
        available_cause_map = available_cause_map.to_dict()["parent_cause_name"]
