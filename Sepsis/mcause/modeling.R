...
save_pre_model_diagnostics <- function(df, dir, covariates) {
  if ("level_1" %in% colnames(df)) {
    for (level in unique(df$level_1)) {
        ...
        ggsave("FILEPATH", width=11, height=8.5)
    }
  }
...
save_betas <- function(model, df, dir, int_cause, fold){
  ...
  write.csv(cbind(covs, tab, exp_tab), "FILEPATH", row.names = FALSE)
}
...
save_diagnostics <- function(dt, dir, int_cause) {
  ...
  out_dir <- dir
  ggsave(
    sprintf("IS_%s_model_version_roc_curve.pdf", int_cause),
    plot = res$roc_curve,
    device = "pdf",
    path = out_dir
  )
  file_name <- sprintf(
    "IS_%s_model_version_confuse_auc.csv", int_cause)
  write.csv(cm, file = "FILEPATH", row.names = FALSE)
  ggsave(
    sprintf("IS_%s_model_version_regularized_resids.pdf", int_cause),
    plot = reg_res_plot,
    device = "pdf",
    path = out_dir
  )
  ...
  write.csv(rmse, file = "FILEPATH", row.names = FALSE)
}