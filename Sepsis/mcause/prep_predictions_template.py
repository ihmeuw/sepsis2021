...
from mcod_prep.utils.covariates import (
    drop_unmodeled_asc_aggregate_ages,
    enforce_redistribution_restrictions,
    get_int_cause_model_covariates,
    merge_covariate,
)
...
def drop_unmodeled_asc(df, cause_meta_df, age_meta_df, yld=False):
    if yld:
        ...
        prefix = "yll"
    add_cause_cols = [f"{prefix}_age_start", f"{prefix}_age_end", "male", "female"]
...
def save_template(
    ...
    age_meta_df = get_ages(force_rerun=True, block_rerun=False)
    detail_ages = get_cod_ages()["age_group_id"].unique().tolist()
    ...
    if end_product == "mortality" and int_cause in MCoDMapper.infectious_syndromes:
        if len(set(model_ages) - set(detail_ages)) != 0:
            ...
            template = template.merge(
                age_detail_map[["agg_age_group_id", "age_group_id"]],
                how="left",
                on="agg_age_group_id",
            )
    ...
    if end_product == "rdp":
        ...
        custom_ch = (
            get_int_cause_hierarchy(int_cause, **kwargs).set_index("level_2")["level_1"].to_dict()
        )
    ...
    for covariate in covariates:
        template = merge_covariate(
            template, covariate, **{"force_rerun": True, "block_rerun": False}
        )
    result = McauseResult(
        int_cause=int_cause,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Save csv that has all age/sex/location/year/causes " "for which to predict."
    )
