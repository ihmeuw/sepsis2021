...
def delineate_source_cols(df):
    df = df.copy()
    df.loc[df["source"].str.contains("full_denom"), "source"] = "SOURCE"
    return df
...
def _fix_SOURCE_denom(df, gbd_df, dem_cols, SOURCE_prop_col):
    df = df.merge(gbd_df[dem_cols + ["gbd_prop"]],
                  on=dem_cols)
    df["adjusted_denom"] = df["total_meningitis"] + (df[f"{SOURCE_prop_col}"] * df["gbd_prop"])
    df.drop(["gbd_prop", "total_meningitis"], axis=1, inplace=True)
    df.rename(columns={"adjusted_denom": "total_meningitis"}, inplace=True)
    return df


def fix_SOURCE_denom(df, gbd_df, dem_cols, pathogen):
    df["total_meningitis"] = df.groupby(dem_cols + ["source"])[
        "value"].transform("sum")

    if pathogen == "Streptococcus pneumoniae":
        df = _fix_SOURCE_denom(df.query(f"pathogen=='{pathogen}'"),
                           gbd_df, dem_cols, SOURCE_prop_col="value")
    else:
        ...
        df = _fix_SOURCE_denom(df.query(f"pathogen_x=='Streptococcus pneumoniae'"),
                           gbd_df, dem_cols, SoURCE_prop_col="strep_value")

    return df


def adjust_SOURCE(df, pathogen, measure):
...
def prep_non_lit_data(pathogen, syndrome, group_cols, value_cols, measure):
    ...
    print_log_message("adjusting source denom for meningitis")
    if measure == "morbidity":
