...
def map_to_estimated_pathogens(df):
    ...
    name_df = pd.read_excel(f"{CONF.get_resource('pathogen_integer_map')}")
...
def add_on_unspecified():
    ...
    ten_map = pd.read_excel("FILEPATH", sheet_name="ICD10_map")
    nine_map = pd.read_excel("FILEPATH", sheet_name="ICD9-map")
